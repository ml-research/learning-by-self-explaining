...
def get_loader(dataset, batch_size, num_workers=8, shuffle=True, drop_last=True):
...
class CLEVR_HANS_EXPL(torch.utils.data.Dataset):
    def __init__(self, base_path, split, perc=1., lexi=False, conf_vers='conf_2'):
        assert split in {
            "train",
            "train_lsx",
            "val",
        ...
        # take only percentage
        scenes = scenes[:int(self.perc * len(scenes))]
        ...
        # of samples
        self.sample_ids = list(np.arange(0, len(self.img_ids)))
    ...
    def scenes_path(self):
        if 'lsx' in self.split:
            tmp = os.path.join(
                self.base_path, 'train', "CLEVR_HANS_scenes_train_{}.json".format(self.split)
            )
            self.split = 'train'
            return tmp
        else:
    ...
    def update_pos_sample_ids(self, class_id):
        rel_ids = np.where(np.array(self.img_class_ids)[self.full_sample_ids] == class_id)[0]
    ...
    def __getitem__(self, item):
        ...
        image_id = self.img_ids[id]

        image = pil_loader(self.fnames[id])
        ...
        objects = self.scenes[id]
        # GT explanation feedback (i.e. human feedback)
        gt_table_expl = self.gt_table_expls[id]
        img_class_id = self.img_class_ids[id]

        proposed_table_expl = torch.tensor([0])
        proposed_table_expl_inv = torch.tensor([0])
        if self.class_expl_feedback_masks is not None:
            proposed_table_expl = self.class_expl_feedback_masks[img_class_id]
            proposed_table_expl_inv = self.class_expl_feedback_masks_inv[img_class_id]

        # remove objects presence indicator from gt table
        objects = objects[:, 1:]

        return image, objects, img_class_id, image_id, img_expl, gt_table_expl, \
               proposed_table_expl, proposed_table_expl_inv
