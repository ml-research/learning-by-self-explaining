...
class AverageMeter(object):
    ...
    def update(self, val, val_pred=None, val_expl=None, n=1):
        self.val = val
        self.sum += val * n
        if val_pred is not None:
            self.val_pred = val_pred
            self.sum_pred += val_pred * n
        if val_expl is not None:
            self.val_expl = val_expl
            self.sum_expl += val_expl * n
        self.count += n
        self.avg = self.sum / self.count
        self.avg_pred = self.sum_pred / self.count
        self.avg_expl = self.sum_expl / self.count
