...
from CUB.dataset import CATEGORY_IDS

# import data_clevr
# import data_kandinsky
from percept import SlotAttentionPerceptionModule, YOLOPerceptionModule, CUBPerceptionModule
from facts_converter import FactsConverter
from nsfr import NSFReasoner
from logic_utils import build_infer_module, build_clause_infer_module
from valuation import SlotAttentionValuationModule, YOLOValuationModule, CUBValuationModule
...
def get_nsfr_model(args, lang, clauses, atoms, bk, bk_clauses, device, train=False):
    if args.dataset_type == 'kandinsky':
        ...
        assert args.fp_cat_attr_dict is not None
        # load category attribute dict
        with open(args.fp_cat_attr_dict, 'rb') as handle:
            cat_attrs_dict = pickle.load(handle)
        PM = CUBPerceptionModule(e=1, d=args.n_attributes, cat_ids=CATEGORY_IDS, device=device)
        VM = CUBValuationModule(lang=lang, device=device, cat_attrs_dict=cat_attrs_dict)
    else:
