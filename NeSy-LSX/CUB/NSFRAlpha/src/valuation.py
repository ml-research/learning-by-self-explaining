...
class YOLOValuationModule(nn.Module):
    ...
    def init_valuation_functions(self, device, dataset=None):
        ...
        vfs['closeby'].load_state_dict(torch.load(
                'NSFRAlpha/src/weights/neural_predicates/closeby_pretrain.pt', map_location=device))
        vfs['closeby'].eval()
        ...
        vfs['online'].load_state_dict(torch.load(
                'NSFRAlpha/src/weights/neural_predicates/online_pretrain.pt', map_location=device))
        vfs['online'].eval()
...
class SlotAttentionValuationModule(nn.Module):
    ...
    def init_valuation_functions(self, device, pretrained):
        ...
        vfs['front'] = v_front
        # v_only_n_objects = OnlyNObjectsValuationFunction(device)
        # vfs['only_n_objects'] = v_only_n_objects

        if pretrained:
            vfs['rightside'].load_state_dict(torch.load(
                'NSFRAlpha/src/weights/neural_predicates/rightside_pretrain.pt', map_location=device))
            vfs['rightside'].eval()
            vfs['leftside'].load_state_dict(torch.load(
                'NSFRAlpha/src/weights/neural_predicates/leftside_pretrain.pt', map_location=device))
            vfs['leftside'].eval()
            vfs['front'].load_state_dict(torch.load(
                'NSFRAlpha/src/weights/neural_predicates/front_pretrain.pt', map_location=device))
            vfs['front'].eval()
            print('Pretrained  neural predicates have been loaded!')
        # return nn.ModuleList([v_color, v_shape, v_in, v_size, v_material, v_rightside,
        #                       v_leftside, v_front, v_only_n_objects]), vfs
        return nn.ModuleList([v_color, v_shape, v_in, v_size, v_material, v_rightside,
                              v_leftside, v_front]), vfs
...
class CUBValuationModule(nn.Module):
    """A module to call valuation functions.
        Attrs:
            lang (language): The language.
            device (device): The device.
    ...
    def __init__(self, lang, device, cat_attrs_dict):
        super().__init__()
        self.lang = lang
        self.device = device
        self.cat_attrs_dict = cat_attrs_dict
    ...
    def init_valuation_functions(self):
        ...
        vfs = {}  # pred name -> valuation function
        v_attrs = []
        for cat in self.cat_attrs_dict.keys():
            v_attr = CUBHasAttrValuationFunction(attr_inds=self.cat_attrs_dict[cat]['ids'])
            v_attrs.append(v_attr)
            vfs[cat] = v_attr
        v_in = CUBInValuationFunction(device=self.device)
        vfs['in'] = v_in

        # return nn.ModuleList([v_color, v_shape, v_in, v_size, v_material, v_rightside,
        #                       v_leftside, v_front, v_only_n_objects]), vfs
        return nn.ModuleList(v_attrs), vfs
    ...
    def term_to_onehot(self, term, batch_size):
        """Ground terms into tensor representations.
            ...
            Return:
                The tensor representation of the input term.
        """
        attrs = self.cat_attrs_dict[term.dtype.name]['attrs']
        return self.to_onehot_batch(attrs.index(term.name), len(attrs), batch_size)
        # if term.dtype.name == 'color':
        #     return self.to_onehot_batch(self.colors.index(term.name), len(self.colors), batch_size)
        # elif term.dtype.name == 'shape':
        #     return self.to_onehot_batch(self.shapes.index(term.name), len(self.shapes), batch_size)
