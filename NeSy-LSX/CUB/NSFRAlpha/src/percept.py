...
class CUBPerceptionModule(nn.Module):
    """
    Dummy perception module.
    """

    def __init__(self, e, d, cat_ids, device, temp=.1):
        super().__init__()
    ...
    def forward(self, x):
        # add object presence variable
        x = torch.cat((x, torch.ones(x.shape[0], 1, device=self.device)), dim=1)
        # add n_obj dimension, even if we only have one object
...
class YOLOPerceptionModule(nn.Module):
    ...
    def __init__(self, e, d, device, train=False):
        ...
        self.model = self.load_model(
            path='NSFRALpha/src/weights/yolov5/best.pt', device=device)
...
class SlotAttentionPerceptionModule(nn.Module):
    ...
    def load_model(self):
        ...
        if self.device == 'cpu':
            ...
            log = torch.load(
                "NSFRAlpha/src/weights/slot_attention/best.pt", map_location=torch.device(self.device))
            ...
            sa_net = SlotAttention_model(n_slots=10, n_iters=3, n_attr=18,
                                         encoder_hidden_channels=64,
                                         attention_hidden_channels=128, device=self.device)
            log = torch.load("NSFRAlpha/src/weights/slot_attention/best.pt")
            sa_net.load_state_dict(log['weights'], strict=True)
            sa_net.to(self.device)
            if not self.train_:
                for param in sa_net.parameters():
