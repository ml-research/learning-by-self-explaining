...
def load_images_and_labels(dataset='clevr-hans3', split='train'):
    ...
    image_paths = []
    labels = []
    folder = 'NSFRAlpha/data/clevr/' + dataset + '/' + split + '/'
...
def load_images_and_labels_negative(dataset='clevr-hans0', split='train'):
    """Load image paths and labels for clevr-hans dataset.
    """
    image_paths = []
    labels = []
...
def __load_images_and_labels(dataset='clevr-hans0', split='train', base=None):
    ...
    labels = []
    if base == None:
        base_folder = 'NSFRAlpha/data/clevr/' + dataset + '/' + split + '/'
    else:
        base_folder = base + 'NSFRAlpha//data/clevr/' + dataset + '/' + split + '/'
...
class CLEVRHans_NEGATIVE(torch.utils.data.Dataset):
    """CLEVRHans dataset.
    The implementations is mainly from https://github.com/ml-research/NeSyConceptLearner/blob/main/src/pretrain-slot-attention/data.py.
    """
